...
class SR_C1C2(object):
    ...
    def _create_placeholder(self):
        pass
        self.input=tf.placeholder(self.dtype,[None,None,None,6],name='input')
        self.target=tf.placeholder(self.dtype,[None,None,None,6],name='output')
    ...
    def conv_test(self,x,hidden_size,bottleneck_size,name,with_last_relu=True):
        x=tf.layers.conv2d(x,bottleneck_size,1,activation=tf.nn.relu,name=name+'_proj')
        if(with_last_relu):
            x=tf.layers.conv2d(x,hidden_size,3,activation=tf.nn.relu,name=name+'_filt')
        else:
